...
const Stories = ({addActivity}) => {
  ...
  const postStory = async e => {
    ...
    if (file) {
      let image = imgUrl.imageUrl.split("/").pop()
      mutation.mutate({ img: image })
  ...
  return (
    <div className="storyPage">
      ...
      <div className="storyDisplay" id="storyDisplay" onClick={()=>{moveToNext()}}>
        ...
        >
          <img src={UPLOAD_URL + currentUser.profilePic} alt=""/>
        ...
        <Link 
          ...
          style={{textDecoration: "none", color: "inherit"}}
        >
          <img src={UPLOAD_URL + story[0].profilePic} alt=""/>
        ...
        {!newStory ?     
          <img src={UPLOAD_URL + story[count].img} alt=""/>
        : <></> }
      ...
      <div className="storiesContainer">
        <div className="stories">
          ...
          <div className="story story1" >
              {myStories.length > 0 ?
              <>
              <img src={UPLOAD_URL + myStories[0].img} alt="" title="My Story" onClick={()=>{
                setStoryOpen(true);
                setNewStory(false)
                setStory(myStories)
                setCount(0)
          ...
          {updatedStories.length > 0 ? updatedStories.map((storyArray, index)=>(
            <div 
              className="story" 
              title={storyArray[0].username + " " + storyArray[0].name} 
              key={index} 
              onClick={()=>{
                setStoryOpen(true);
                setNewStory(false)
              ...
              <img src={UPLOAD_URL + storyArray[0].img} alt="" />
            </div>
          ))  : <></>}   
        </div> 
      </div>
